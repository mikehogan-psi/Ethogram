...
for mouse = mice_to_analyse
    ...
    if ~exist(save_path, 'file')
        ...
        % Store raw trial numbers for data partitioning
        X_table.TrialRaw = X_table.Trial;

        % trial number within each type (looms 1..20, flashes 1..20)
        ...
        pseudoR2 = nan(nCells,1);
        weights_per_cell = cell(nCells, 1);   % one entry per cell
        coef_names = {};                      % will fill once from first successful model
    
        pseudoR2_shuff_all = cell(nCells,1);     % full distribution per cell
        pseudoR2_shuff_p95 = nan(nCells,1);      % e.g., 95th percentile per cell
        pseudoR2_shuff_mean = nan(nCells,1);     % optional
        ...
        for c = 1:nCells
            ...
            predictor_vars = {'Grooming','Rearing','Darting','Freezing','Velocity', ...
                 'TimeBin','Trial','TrialIdentifier','StimOn','StimOn_x_Type'};
            X_cell_model = X_cell(:, predictor_vars);
            
            trial_raw  = X_cell.TrialRaw;         % per-row trial id (for this cell)
            trial_type = X_cell.TrialIdentifier;  % 1=loom, 0=flash (for this cell)

            y_cell = y(idx_all);          
            
            % unique trial IDs per type
            loom_trials  = unique(trial_raw(trial_type == 1));
            flash_trials = unique(trial_raw(trial_type == 0));
            
            % hold out 20% (at least 1)
            n_hold_loom  = max(1, round(0.2 * numel(loom_trials)));
            n_hold_flash = max(1, round(0.2 * numel(flash_trials)));
            
            % sample trials (randperm needs indices, not the vector itself)
            test_loom  = loom_trials(randperm(numel(loom_trials),  n_hold_loom));
            test_flash = flash_trials(randperm(numel(flash_trials), n_hold_flash));
            
            test_trials = [test_loom; test_flash];
            
            % bin-level masks from trial IDs
            test_idx  = ismember(trial_raw, test_trials);
            train_idx = ~test_idx;

            X_train = X_cell_model(train_idx, :);
            y_train = y_cell(train_idx);
            
            X_test  = X_cell_model(test_idx, :);
            y_test  = y_cell(test_idx);


            if c == 1
                fprintf('Holdout: %d loom, %d flash trials | Test bins = %d\n', ...
                    numel(test_loom), numel(test_flash), sum(test_idx));
            end
            ...
            % ---- reject non-converged or degenerate fits ----            
            if isprop(mdl,'ConvergenceInfo')
                if isfield(mdl.ConvergenceInfo,'Converged') && ~mdl.ConvergenceInfo.Converged
                    pseudoR2(c) = NaN;
                    continue
                end
            end
            ...
            if any(~isfinite(beta)) || any(abs(beta) > 50)   % threshold is pragmatic
                pseudoR2(c) = NaN;
                continue
            end
            
            lambda_test = predict(mdl, X_test);
            if any(~isfinite(lambda_test)) || any(lambda_test > 1e6)  % pragmatic explosion check
                pseudoR2(c) = NaN;
                continue
            end
            ...
            ratio = D_full_test / D_null_test;

            if ~isfinite(ratio) || D_null_test < 1e-3   % choose threshold (1e-3, 1e-2)
                pseudoR2(c) = NaN;                 % undefined / unstable
            else
                pseudoR2(c) = max(-1, 1 - ratio);  % clamp at -1
            end
                ...
                for s = 1:nShuff
                    ...
                    try
                        mdl_s = fitglm(X_cell_model(train_idx,:), y_shuff(train_idx), ...
                            'Distribution','poisson','Options',opts);
            
                        lambda_s = predict(mdl_s, X_cell_model(test_idx,:));
            
                        D_full_s = poissonDeviance(y_shuff(test_idx), lambda_s);
            
                        mu_null_s = mean(y_shuff(train_idx));
                        D_null_s  = poissonDeviance(y_shuff(test_idx), mu_null_s * ones(sum(test_idx),1));
            
                        ratio_s = D_full_s / D_null_s;
            
                        if isfinite(ratio_s) && D_null_s > 1e-3
                            pseudoR2_shuff(s) = max(-1, 1 - ratio_s);   % use same clamp logic
                        end
                    catch
                        % leave NaN
                    end
        ...
        X_table_full = removevars(X_table_full, ["TrialRaw","LoomON","FlashON"]);
        X_table_full.SpikeCount = y;
        X_table_full.CellID = cell_ids;

        if ~do_shuffle
        % Save per-mouse results
        save(save_path, 'X_table_full', 'pseudoR2', ...
            'unique_cells', 'weights_per_cell', 'coef_names');
        disp([save_name ' saved to ' save_path '!'])
        else
        save(save_path, 'X_table_full', 'pseudoR2', ...
            'pseudoR2_shuff_all', 'pseudoR2_shuff_p95', 'pseudoR2_shuff_mean', ...
            'unique_cells', 'weights_per_cell', 'coef_names');
        disp([save_name ' saved to ' save_path '!'])
        end
