...
% Concatenate and provide session identifier
% Choose session label consistently (use string)
session = string(session);

cluster_assignments.Session = repmat("Extinction", height(cluster_assignments), 1);
cluster_assignments_retention.Session = repmat("Renewal", height(cluster_assignments_retention), 1); % or "Retention" if that's true

cluster_assignments_all = [cluster_assignments; cluster_assignments_retention];

% Filter assignments to the session you're plotting
cluster_assignments = cluster_assignments_all(cluster_assignments_all.Session == session, :);
...
for cluster = 1:num_clusters
    ...
    xline(13.3, 'k--', 'LineWidth', 1.5)
    % ylim([0 15])
    sgtitle("PSTHs across loom trials: " + session)
