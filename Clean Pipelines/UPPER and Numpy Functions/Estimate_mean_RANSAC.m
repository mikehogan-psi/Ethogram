function[mean_pose_3D] = Estimate_mean_RANSAC(X_3D,graph)
...
mean_pose_3D=New_ref{p};
mean_pose_3D =  mean_pose_3D-repmat(mean(mean_pose_3D),Np,1);
coeff = pca(mean_pose_3D(:,1:2));
x_align = [zeros(Np,1) mean_pose_3D(:,1:2)*coeff(:,1)];
[~,mean_pose_3D(:,1:2)] = procrustes(x_align,mean_pose_3D(:,1:2),'reflection',false,'scaling',false);
