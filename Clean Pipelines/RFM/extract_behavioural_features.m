...
% For SSM_mouse1_to_9, eigenpose 1: head up/down, 2: head side/side, 3:
% elongation/hunching

% For SSMSSM_3D_implant_mouse1_head_fixed, eigenpose 1: head side/side, 2: head up/down,  3:
% elongation/hunching
num_frames = size(b, 2);
...
'mean(b_window(1, :))';                          % Feature 7:  Average "looking up/down" (General vertical posture trend)
'var(b_window(1, :))';                           % Feature 8:  Variability in "looking up/down" posture (vertical motion dynamism / general fluctuation)
'mean(diff(b_window(1, :)))';                    % Feature 9:  Trend of change in "looking up/down" posture (rate and  direction of vertical motion dynamism)
...
elseif strcmp(behaviour, 'Rearing')   
    feature_strings_idx = [1 21, 24, 25, 26, 27];
    window_size = 4;
