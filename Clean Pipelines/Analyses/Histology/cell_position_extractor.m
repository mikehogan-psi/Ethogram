...
data_folders = cell(num_mice, 1);

% Get depth folders for specified session
for mouse = 1:num_mice
    mouse_name = mouse_files(mouse).name;
    mouse_path = mouse_files(mouse).folder;
...
%% Assign cells a best channel based on max ptp amplitude

for mouse = 1:num_mice
    if ismember(mouse, mice_to_exclude)
        disp(['Mouse ' num2str(mouse) ' excluded'])
        continue
    else
