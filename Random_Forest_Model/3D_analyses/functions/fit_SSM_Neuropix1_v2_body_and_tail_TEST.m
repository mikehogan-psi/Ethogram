function[b,Xfit,R,T] = fit_SSM_Neuropix1_v2_body_and_tail_TEST(X,W,mean_pose,lambda,P,alpha_reg,var_res,options)
Nshape = numel(lambda);
[Np,Nd,Nt] = size(X);
b0 = zeros(1,Nshape);
...
if isempty(ind_body) && isempty(ind_head)
    % Treat all landmarks equally if no head/body distinction is given
    ind_all = 1:Np;
    for t = 1:Nt
        if any(W(:,t) > 0)
            ind1 = find(W(:,t) > 0);
            btemp = fminunc(@(btemp) fit_SSM(btemp, X(ind1,:,t), mean_pose(ind1,:), lambda, P(ind1,:,:), alpha_reg, var_res), ...
                            b0, optimoptions('fminunc','Display','none'));
            if all(abs(btemp)<10*std(lambda))
                b(t,:) = btemp;
                [~, ~, R(:,:,t), T(t,:)] = fit_SSM(btemp, X(ind1,:,t), mean_pose(ind1,:), lambda, P(ind1,:,:), alpha_reg, var_res);
                Xfit(:,:,t) = mean_pose;
                for m = 1:Nshape
                    Xfit(:,:,t) = Xfit(:,:,t) + b(t,m)*P(:,:,m);
                end
                Xfit(:,:,t) = Xfit(:,:,t)*R(:,:,t) + repmat(T(t,:),Np,1);
            end
        end
        if mod(t,100) == 0
            fprintf('Pose %d of %d\n', t, Nt);
