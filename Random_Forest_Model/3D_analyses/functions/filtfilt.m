function y = filtfilt(b,a,x)
    ...
    coder.internal.assert((isUnderlyingType(b,'double') || isUnderlyingType(b,'single')) && ...
                          (isUnderlyingType(a,'double') || isUnderlyingType(a,'single')) && ...
                          (isUnderlyingType(x,'double') || isUnderlyingType(x,'single')), ...
                          'signal:filtfilt:NotSupported');

    if (isempty(b) || isempty(a) || isempty(x))
        % Trick to accomodate other array types (e.g. gpuArray)
        y = cast([],"like",b) + cast([],"like",a) + cast([],"like",x);
        return
    end
...
function y = efiltfilt(b,a,x)    
    ...
    % Input validation
    % double and single precision is supported
    ...
    % if any of the inputs is of single precision, all the computations
    % occur in single precision.
    [b1,a1,x1] = signal.internal.utilities.castInputToSingle(b,a,x);

    % If input data is a row vector, convert it to a column.
    xIsRow = isrow(x1);
    if xIsRow
        % to avoid transposing N-dim array in code generation
        xCol = reshape(x1,size(x1,2),1);
    else
        xCol = x1;
    end
    ...
    % generation.
    odt = zeros('like',a2) + zeros('like',b2) + zeros('like',xCol);

    % Filter the data
    if Nchans==1
        if Npts<10000
            yCol = ffOneChanCat(b2,a2,xCol,zi,nfact,L,odt);
        else
            yCol = ffOneChan(b2,a2,xCol,zi,nfact,L,odt);
        end
        
    else
        yCol = ffMultiChan(b2,a2,xCol,zi,nfact,L,odt);
    end
