function feature_extractor_new(video_path, mat_file_path, output_folder, frame_features_strings, window_size)
    % Load the video
    ...
    function frame_features = calculate_features(frame_idx)
        ...
        % frame_features_all = [
        %                       mean(b_window(1, :));                             % Feature 1
        %                       var(b_window(1, :));                              % Feature 2
        %                       mean(diff(b_window(1, :)));                       % Feature 3
        %                       mean(b_window(2, :));                             % Feature 4
        %                       var(b_window(2, :));                              % Feature 5
        %                       mean(diff(b_window(2, :)));                       % Feature 6
        %                       mean(b_window(3, :));                             % Feature 7
        %                       var(b_window(3, :));                              % Feature 8
        %                       mean(diff(b_window(3, :)));                       % Feature 9
        %                       mean([0, sqrt(sum(diff(T_window, 1, 2).^2))]);    % Feature 10
        %                       mean([0, abs(diff(A_window))])                    % Feature 11
        %                      ];
