    ...
    def __init__(self, input_dim, hidden_dim_list, num_cells, dropout=0.):        
        ...
        for i in range(1, len(hidden_dim_list)):
            self.hidden_layers.append(
                nn.Linear(hidden_dim_list[i-1], hidden_dim_list[i]))
            self.dropouts.append(nn.Dropout(dropout))
        ...
        #Cell-specific heads
        self.heads = nn.ModuleList([nn.Linear(final_hidden_dim, 1) 
    ...
    def train_model(self, dataloader, max_epochs=1000, learning_rate=1e-3, 
        ...
        for epoch in range(max_epochs):
            ...
            if epoch in checkpoints:
                torch.save(self, net_name + "_checkpoint" + str(epoch) + ".pth")           
    ...
    def test_model(self, dataloader, cell_id): 
        ...
        with torch.no_grad():
            for x_batch, cell_batch, y_batch in dataloader:
                ...
                y_pred.append(self.forward(x_batch[indices,:], 
                                           cell_batch[indices]))
        y = torch.cat(y,dim=0)
