...
for n = 1:N_neurons % loop trough each neuron
   ...
   for m = 1:2 % run GLMM for 1 -> during stim period, 2 -> post-stim period  
     ...
     % get binsize depending on how many bins you want
       num_bins = num_bins(m);                  % define number of bins
       bin_size = period_length(m) / num_bins;  % get binsize dependent on length of investigated period (if longer, greater bin size required)
...
for g = 1:numel(group_val)
    for n = 1:Nval
        ...
        trial_idx = data_binned.Trial == trial_val(n);
...
for g = 1:numel(group_val)
    for n = 1:Nval
        group_idx = data_binned.Group == group_val(g);
        time_idx = data_binned.TimeBin == bin_val(n);
